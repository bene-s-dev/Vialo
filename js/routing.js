...
export const Routing = {
   ...
   */
  async searchAddress(query, viewbox = null) {
  ...
  async getRouteBRouter(points, profile, options = null) {
    ...
    const buildUrl = (overrideKeys) => {
      let url = `https://brouter.de/brouter?lonlats=${encodeURIComponent(lonlats)}&profile=${profile}&alternativeidx=0&format=csv`;
      if (options && overrideKeys) {
        overrideKeys.forEach(k => {
          if (options[k] !== undefined) {
            url += `&profile:${k}=${options[k] ? '1' : '0'}`;
          }
        });
      }
      return url;
    ...
    // Determine initial keys to try
    let initialKeys = [];
    if (options) {
      if (profile === 'hiking') {
        initialKeys = ['allow_steps', 'allow_ferries'];
      } else {
        initialKeys = [
          'allow_steps', 'allow_ferries', 'ignore_cycleroutes', 'stick_to_cycleroutes',
          'use_proposed_cycleroutes', 'avoid_unsafe', 'add_beeline', 'consider_noise',
          'consider_river', 'consider_forest', 'consider_town', 'consider_traffic',
          'consider_elevation'
        ];
      }
    }

    let url = buildUrl(initialKeys);
    let response;

    try {
      response = await fetch(url);
      
      // Fallback 1: If 500 and we tried all keys for trekking, fallback to standard trekking keys
      if (!response.ok && response.status === 500 && profile === 'trekking' && initialKeys.length > 5) {
        console.warn('BRouter returned 500. Retrying with standard trekking keys only...');
        const stdKeys = ['allow_steps', 'allow_ferries', 'ignore_cycleroutes', 'stick_to_cycleroutes', 'avoid_unsafe'];
        url = buildUrl(stdKeys);
        response = await fetch(url);
      }
      
      // Fallback 2: If still not OK, try without any overrides
      if (!response.ok && response.status === 500) {
        console.warn('BRouter still returned 500. Retrying without any parameter overrides...');
        url = buildUrl([]);
        response = await fetch(url);
      }
      ...
      for (const seg of parsed.segments) {
        ...
        const details = getSurfaceDetails(seg.surface);
